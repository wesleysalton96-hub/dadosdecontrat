...
def pagina_e_ficha_registro(texto):
    ...
    A ideia aqui é evitar quebrar o kit em páginas intermediárias
    que eventualmente contenham "Registro de Empregado" em cabeçalho,
    rodapé ou outro contexto.

    Regras usadas:
    - contém "REGISTRO DE EMPREGADO"
    - contém "EMPREGADOR"
    - contém "FUNCIONÁRIO" ou "FUNCIONARIO"
    - contém "NOME"
    """
    if not texto:
        return False
...
def localizar_kits_no_pdf(doc):
    ...
    REGRA:
    - um kit começa SOMENTE quando a página é reconhecida como
      Ficha de Registro.
    - todas as páginas seguintes pertencem ao mesmo kit
      até a próxima Ficha de Registro.
    """
    inicios = []
...
def main():
    ...
    for idx, (inicio, fim) in enumerate(kits, start=1):
        try:
            ...
            # Nome é lido SOMENTE da primeira página do kit
            ...
            # Salva todas as páginas do kit:
            # da Ficha de Registro até a página anterior à próxima Ficha
