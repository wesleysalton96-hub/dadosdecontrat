...
# Possíveis nomes para a coluna do colaborador
...
# Possíveis nomes para a coluna da pasta
...
def extrair_nome_da_pagina(texto):
    ...
    # CASO 1 e 2: Nome: Fulano / Nome : Fulano
    for linha in linhas:
...
def pagina_e_ficha_registro(texto):
    """
    Verifica se a página é a primeira página do kit (Ficha de Registro).
    """
    if not texto:
        return False
...
def localizar_kits_no_pdf(doc):
    """
    Um kit começa SOMENTE quando a página é reconhecida como Ficha de Registro.
...
# ============================================================
# FUNÇÕES PARA IDENTIFICAR COLUNAS DINAMICAMENTE
# ============================================================

def encontrar_coluna_por_cabecalho(df, possiveis_nomes):
    """
    Procura automaticamente no cabeçalho da planilha uma coluna
    que combine com algum dos nomes esperados.
    Retorna o nome real da coluna no DataFrame ou None.
    """
    mapa_colunas = {}

    for coluna in df.columns:
        col_norm = normalizar_texto(coluna)
        mapa_colunas[col_norm] = coluna

    for nome_esperado in possiveis_nomes:
        nome_norm = normalizar_texto(nome_esperado)
        if nome_norm in mapa_colunas:
            return mapa_colunas[nome_norm]

    return None


def escolher_coluna_manual(df, titulo):
    """
    Se a coluna não for encontrada automaticamente,
    mostra os cabeçalhos disponíveis e pede ao usuário
    que escolha pelo número.
    """
    print("\n" + "=" * 70)
    print(titulo)
    print("=" * 70)
    print("Cabeçalhos encontrados na planilha:\n")
    ...
    while True:
        escolha = input("\nDigite o número da coluna desejada: ").strip()

        if not escolha.isdigit():
            print("Valor inválido. Digite apenas o número da coluna.")
            continue
...
def carregar_base_excel(caminho_excel):
    """
    Carrega a base do Excel e encontra dinamicamente:
    - a coluna do nome
    - a coluna da pasta

    Se não encontrar automaticamente, pergunta ao usuário.
    """
    header = 0 if PLANILHA_TEM_CABECALHO else None
    df = pd.read_excel(caminho_excel, header=header)

    if not PLANILHA_TEM_CABECALHO:
        raise ValueError(
            "A versão 2.3 exige cabeçalho na planilha para localizar as colunas dinamicamente."
        )

    # --------------------------------------------------------
    # Tenta encontrar a coluna do nome automaticamente
    # --------------------------------------------------------
    coluna_nome = encontrar_coluna_por_cabecalho(df, POSSIVEIS_CABECALHOS_NOME)

    if coluna_nome is None:
        coluna_nome = escolher_coluna_manual(
            df,
            "Não encontrei automaticamente a coluna do NOME do colaborador."
        )

    # --------------------------------------------------------
    # Tenta encontrar a coluna da pasta automaticamente
    # --------------------------------------------------------
    coluna_pasta = encontrar_coluna_por_cabecalho(df, POSSIVEIS_CABECALHOS_PASTA)

    if coluna_pasta is None:
        coluna_pasta = escolher_coluna_manual(
            df,
            "Não encontrei automaticamente a coluna da PASTA de destino."
        )
    ...
    return base, linhas_invalidas, coluna_nome, coluna_pasta
...
def main():
    ...
    try:
        base_colaboradores, linhas_invalidas, coluna_nome, coluna_pasta = carregar_base_excel(caminho_excel)
    except Exception as e:
        escrever_log(caminho_log, f"[ERRO] Falha ao carregar Excel: {str(e)}")
        return
    ...
    escrever_log(caminho_log, f"Coluna de nome identificada: {coluna_nome}")
    escrever_log(caminho_log, f"Coluna de pasta identificada: {coluna_pasta}")
    ...
    escrever_log(caminho_log, "=" * 80)
    escrever_log(caminho_log, f"Coluna de nome utilizada: {coluna_nome}")
    escrever_log(caminho_log, f"Coluna de pasta utilizada: {coluna_pasta}")
